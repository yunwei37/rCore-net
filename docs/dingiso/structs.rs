...
pub trait Socket: Send + Sync + Debug {
    /// read as many as possible from receive buffer(rx_buffer) , and fill a slice(data) from it
    fn read(&self, data: &mut [u8]) -> (SysResult, Endpoint);
    /// enqueue the content from data to the send buffer(tx_buffer), and send it to the remote endpoint
    fn write(&self, data: &[u8], sendto_endpoint: Option<Endpoint>) -> SysResult;
    /// poll the state of the SOCKETS
    fn poll(&self) -> (bool, bool, bool); // (in, out, err)
    /// connec to the remote endpoint
    fn connect(&mut self, endpoint: Endpoint) -> SysResult;
    /// bind self with the endpoint
    fn bind(&mut self, _endpoint: Endpoint) -> SysResult {
        Err(SysError::EINVAL)
    }
    /// check whether listen , if not start listening on the local endpoint
...
impl TcpSocketState {
    /// new a TcpSocket and assign the handle to the TcpSocketState
    pub fn new() -> Self {
...
impl Socket for TcpSocketState {
    /// used for TcpSocket to read octets from the buffer
    /// dequeue a sequence as many as possible of recieved octets (rx_buffer) , and fill the slice (data) from it
    fn read(&self, data: &mut [u8]) -> (SysResult, Endpoint) {
    ...
    ///  Enqueue a sequence of octets (tx_buffer) to be sent, and fill it from the slice (data)
    ...
    /// wait for socket states and then connect to endpoint through a ephemeral port
    fn connect(&mut self, endpoint: Endpoint) -> SysResult {
        let mut sockets = SOCKETS.lock();
        let mut socket = sockets.get::<TcpSocket>(self.handle.0);
    ...
    /// get an ephemeral port and bind with the endpoint
    ...
    /// check whether listening or not , if not start listening on the local endpoint
    fn listen(&mut self) -> SysResult {
        ...
        // start listening on the local endpoint
    ...
    /// create a new socket from listen queue, and listen the endpoint
    fn accept(&mut self) -> Result<(Box<dyn Socket>, Endpoint), SysError> {
        ...
        loop {
            ...
            if socket.is_active() {
                ...
                let new_socket = {
                    ...
                    // add a socket to the set with the reference count 1
    ...
    ///return the local endpoint of TcpSocket
    fn endpoint(&self) -> Option<Endpoint> {
        self.local_endpoint
            .clone()
            .map(|e| Endpoint::Ip(e))
    ...
    /// return the remote endpoint of TcpSocket
    fn remote_endpoint(&self) -> Option<Endpoint> {
        let mut sockets = SOCKETS.lock();
        let socket = sockets.get::<TcpSocket>(self.handle.0);
        if socket.is_open() {
...
impl UdpSocketState {
    /// new one socket with rx_buffer and tx_buffer for UdpSocket and give the handle to the UdpSocketState
    pub fn new() -> Self {
...
impl Socket for UdpSocketState {
    /// used for UdpSocket to read octets from the buffer
    /// dequeue a sequence as many as possible of recieved octets (rx_buffer) , and fill the slice (data) from it
    fn read(&self, data: &mut [u8]) -> (SysResult, Endpoint) {
        loop {
            let mut sockets = SOCKETS.lock();
    ...
    /// poll the state of the UdpSocket
    fn poll(&self) -> (bool, bool, bool) {
        let mut sockets = SOCKETS.lock();
    ...
    /// set the remote endpoint for Udp
    fn connect(&mut self, endpoint: Endpoint) -> SysResult {
        if let Endpoint::Ip(ip) = endpoint {
    ...
    /// bind the Udp Socket to the endpoint
    fn bind(&mut self, endpoint: Endpoint) -> SysResult {
        let mut sockets = SOCKETS.lock();
        let mut socket = sockets.get::<UdpSocket>(self.handle.0);
        if let Endpoint::Ip(ip) = endpoint {
    ...
    /// return the end point of the Udp Socket
    fn endpoint(&self) -> Option<Endpoint> {
        let mut sockets = SOCKETS.lock();
        let socket = sockets.get::<UdpSocket>(self.handle.0);
    ...
    /// return the remote endpoint of the Udp Socket
    fn remote_endpoint(&self) -> Option<Endpoint> {
        self.remote_endpoint.clone().map(|e| Endpoint::Ip(e))
    }
    // Clone the Socket with the box packaged
    fn box_clone(&self) -> Box<dyn Socket> {
        Box::new(self.clone())
    }
}
...
impl Socket for RawSocketState {
    /// used for RawSocket to read octets from the buffer
    /// dequeue a sequence as many as possible of recieved octets (rx_buffer) , and fill the slice (data) from it
    fn read(&self, data: &mut [u8]) -> (SysResult, Endpoint) {
        loop {
            let mut sockets = SOCKETS.lock();
    ...
    /// used for RawSocket to send data to the sendto_endpoint ,
    /// Enqueue a sequence of octets (tx_buffer) to be sent, and fill it from the slice (data)
    fn write(&self, data: &[u8], sendto_endpoint: Option<Endpoint>) -> SysResult {
